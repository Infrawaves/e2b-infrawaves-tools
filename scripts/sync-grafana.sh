...
# - 将 JSON 中的 ${DS_PROMETHEUS} 替换为真实的 UID
...
for f in "${files[@]}"; do
  echo
  echo "=== $f ==="
  payload="$(DASH_FILE="$f" DS_UID="$ds_uid" FOLDER_UID="$folder_uid" python3 -c "
import json, os, sys
text = open(os.environ['DASH_FILE']).read()
# 把模板占位符 \${DS_PROMETHEUS} 替换为运行时解析到的真实 UID
text = text.replace('\${DS_PROMETHEUS}', os.environ['DS_UID'])
d = json.loads(text)
d['id'] = None
