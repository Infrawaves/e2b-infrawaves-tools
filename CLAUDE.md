...
- `checkFirecracker.go` → `e2b_fc_process_*`(per-process 带 `vm_kind`=build/instance),扫 `/proc`,从 `--api-sock` 路径解析 `sandbox_id`。build VM 不单独出计数/卡死指标(瞬时 fc 快照语义不准 + 采样混叠);可见性靠 `e2b_fc_process_*{vm_kind="build"}`,卡死用 `e2b_fc_process_uptime_seconds{vm_kind="build"}` 在告警侧判阈值(同 threads/open_fds 模式)
- `checkSandboxLeak.go` → `e2b_sandbox_*` + `e2b_orchestrator_*`,默认连本机 `127.0.0.1:9090` orchestrator gRPC,raw codec + `protowire` 手工解析。leak=A\B 会**剔除 build 前缀**(`buildSandboxPrefix="b"`,对应上游 `InstanceBuildPrefix`)的 sandbox_id——build VM 起真实 fc 但不进 `SandboxService.List`,不算泄露(issue #12);build VM 的计数/卡死指标改由 `checkFirecracker.go` 上报
...
- `e2b_fc_*` — Firecracker 进程视角,labels 含 `node_ip` + `sandbox_id` + `pid` + `vm_kind`(`build`/`instance`,按 sandbox_id 前缀判定,用于区分 template build VM 与用户沙箱;节点级 `count`/`parse_errors`/`state_count` 不带此标签)
